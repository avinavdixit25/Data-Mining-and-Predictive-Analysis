...
# The infant twins are the Baclini sisters
...
# Mrs. Solomon (Latifa Qurban) Baclini is their parent.
...
dfTit %>% 
  mutate(familyOnBoard = sibsp + parch + 1) %>% 
  arrange(desc(familyOnBoard))

# The Sage family is the most crowded one onboard.
...
dfTit %>% 
  ...
  summarize(minFare = min(fare, na.rm = TRUE), 
...
dfTitNew <- dfTit %>% 
  ...
  group_by(ticket) %>% 
  summarize(costPerFamily = sum(fare, na.rm = TRUE),
            familyMembersCount = n(),
            survivedFamilyMembers = sum(survived, na.rm = TRUE)) %>% 
  arrange(desc(costPerFamily)) %>% 
  ungroup()
distinct(dfTitNew, .keep_all = TRUE)
...
fareDist <- 
  ...
  group_by(sex) %>%
  ungroup() %>%
  ggplot(aes(x=sex, y=fare)) + geom_boxplot(aes(fill=factor(survived)))
